...
int main(int argc, char* argv[]) {
    ...
    if(argc != 2)
    {
        std::cout << "Insira dois arquivos: [Tx.txt]  [Rx.txt]" << std::endl;
    }
    std::string file1(argv[1]);
    std::string file2(argv[2]);
    std::cout << "files:" << file1 << "-" << file2 << std::endl;
    std::ifstream TxLog(file1);
    std::ifstream RxLog(file2);
    if (!(TxLog.is_open() || RxLog.is_open())){
